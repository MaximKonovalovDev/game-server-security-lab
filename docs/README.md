...
| Doc | What it is |
|---|---|
| **[docs/ARSENAL.md](ARSENAL.md)** | Frameworks & master workflow (server-authoritative), reference repos (OWASP GSF, Certael, oomph, UltimateAntiCheat...), full tool layers, skill roadmap, immediate next actions. GitHub research 2026. |
| **[docs/TOOLCHAIN.md](TOOLCHAIN.md)** | Layer-by-layer tool selection (watch / attack / know-the-enemy / defend / sandbox) with minimum viable kit + hard rules. Curated from the book + GitHub. |
| **[docs/BOOK-EXTRACT.md](BOOK-EXTRACT.md)** | Curated picks from `trimstray/the-book-of-secret-knowledge`, organized by phase: capture/craft, pentest arsenal, hardening, daily news, shell one-liners. |
