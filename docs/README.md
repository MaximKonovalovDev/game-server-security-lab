...
| **[docs/SERVICES-PLATFORM.md](SERVICES-PLATFORM.md)** | From lab to business: 4 service tiers (website check / server hardening / game hardening / MCP hardening), fleet architecture (agentos sandboxed agents), AI ops via MCP (hexstrike), findings→fix→verify delivery loop, risks, MVP path. Engine: tools/services/scan.py + nuclei via official Docker image (no native binary — Defender false positive on the hash-verified build). |
| **[docs/MONETIZE.md](MONETIZE.md)** | 8 earning directions ranked by fit/time-to-money/ceiling: MCP/AI plugin security audits (THE niche — publishing FINDINGS-MCP-001 for rep), game netcode reviews for indies, bug bounty (MCP corner), productized site scans ($50-150 + retainer, APTRS backend), SecurityPack upsell, nuclei MCP templates, courses, affiliates. Phased $0 path + honest risks. |
| **[docs/FIRM-PROPOSAL.md](FIRM-PROPOSAL.md)** | The full solo-firm plan: boutique positioning across 3 verticals (web/server, game netcode, MCP/AI), the disclosure model done legally (authorized free scans + responsible disclosure + signed paid engagements), service catalog with pricing + monthly retainers (the recurring engine), content capture machine (firm site + 2 niche hubs + YouTube 3-format shows), firm backend (APTRS, templates), 90-day launch plan, KPIs, honest risks. |
| **[docs/PEER-INTEL.md](PEER-INTEL.md)** | Reddit research on how solo security practitioners actually run shops: the prison AMA (report TONE became extortion charges — the standing legal cautionary tale), scoping-questionnaire pricing + tier tables (boutique owner thread, 11 days old), insurance/incorporation floor, the 10-lessons (speak money not stats, exec-readable reports), AI-practitioner skepticism (pitch = automate the 80%, human on deep findings), MCP security questions live on AskNetsec today, solo burnout caps. Corrects FIRM-PROPOSAL in 6 concrete ways. |
